...
export const loginAdmin = async (request) => {
    try {
        ...
        // Update Last Login
        await AdminUser.updateOne({ _id: admin._id }, { lastLogin: new Date() });

        // JWT
        const token = jwt.sign({ id: admin._id, role: admin.role, email: admin.email }, ENV.JWT_ADMIN_SECRET, { expiresIn: `${ENV.COOKIE_EXPIRE}d` });
