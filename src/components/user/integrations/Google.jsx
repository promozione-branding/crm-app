...
export default function Google() {
    ...
    return (
        <div className="space-y-5">
            <div>
                <h2 className="text-base font-semibold text-app">Google Ads</h2>

                <p className="text-xs text-muted mt-1">Connect Google Ads to view campaign performance and analytics.</p>
            </div>
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div className="flex items-center gap-3 min-w-0">
                            <div className="w-12 h-12 rounded-xl border border-app bg-surface flex items-center justify-center shrink-0 overflow-hidden">
                                <img src="/logos/google.png" alt="Google logo" className="w-8 h-8 object-contain" />
                            </div>

                            <div className="min-w-0">
                                <h3 className="font-semibold text-app text-sm">Google Ads</h3>

                                <div className="flex items-center gap-1.5 mt-1">
                                    <span className={`w-1.5 h-1.5 rounded-full ${connected ? 'bg-green-500' : 'bg-blue-500'}`} />
                    ...
                    {connected && (
                        <div className="mt-5">
                            <div className="flex items-center justify-between mb-3">
                                <h4 className="text-xs font-semibold text-app">Google Ads Accounts</h4>
                            ...
                            {accountsLoading ? (
                                <div className="border border-app rounded-xl p-4 text-center">
                                    <p className="text-xs text-muted">Loading Google Ads accounts...</p>
                                </div>
                            ) : accounts.length === 0 ? (
                                <div className="border border-app rounded-xl p-4">
                                    <p className="text-xs text-muted">No Google Ads accounts found.</p>
                                </div>
                            ) : (
                                <div className="space-y-2">
                                    {accounts.map((account) => (
                                        <div key={account.customerId} className="border border-app rounded-xl p-3 flex items-center justify-between gap-3">
                                            <div className="min-w-0">
                                                <p className="text-xs font-medium text-app truncate">{account.name || 'Unnamed Account'}</p>

                                                <p className="text-[10px] text-muted mt-1">Customer ID: {account.customerId}</p>
                                            ...
                                            <button className="shrink-0 px-3 h-8 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-medium">
                                                Select
                                            </button>
                                        </div>
                                    ))}
