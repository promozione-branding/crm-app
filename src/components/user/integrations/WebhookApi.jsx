...
export default function WebhookApi() {
    ...
    return (
        <div className="space-y-5">
            <div>
                <h2 className="text-base font-semibold text-app">Webhook API</h2>

                <p className="text-xs text-muted mt-1">Connect WordPress, custom websites, landing pages and other platforms to receive leads automatically.</p>
            </div>
            ...
            <div className="max-w-3xl bg-app border border-app rounded-2xl p-5">
                ...
                <div className="flex items-center gap-3 pb-5 border-b border-app">
                    ...
                    <div>
                        <h3 className="text-sm font-semibold text-app">API Configuration</h3>

                        <p className="text-xs text-muted mt-1">Use these credentials to send leads to your CRM.</p>
                    </div>
                </div>
                ...
                <div className="space-y-5 mt-5">
                    {/* Webhook URL */}
                    <div>
                        <label className="text-xs font-medium text-app">Webhook URL</label>
                    ...
                    {/* API KEY */}
                    <div>
                        <label className="text-xs font-medium text-app">API Key</label>
                        ...
                        {newApiKey && (
                            <p className="text-[11px] text-amber-500 mt-2">Copy this API key now. For security, the complete key will not be shown again.</p>
                        )}
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-4 p-4 rounded-xl bg-surface border border-app">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-xs font-medium text-app">{status === 'active' ? 'API Active' : 'API Not Configured'}</p>
            ...
            <div className="max-w-3xl bg-app border border-app rounded-2xl p-5">
                <h3 className="text-sm font-semibold text-app">Request Format</h3>

                <p className="text-xs text-muted mt-1">Send a POST request with your API key in the Authorization header.</p>

                <div className="mt-4 space-y-3">
                    <div>
                        <p className="text-[11px] text-muted mb-1">Method</p>

                        <div className="px-3 py-2 rounded-lg bg-surface border border-app text-xs text-app">POST</div>
                    </div>

                    <div>
                        <p className="text-[11px] text-muted mb-1">Authorization</p>

                        <div className="px-3 py-2 rounded-lg bg-surface border border-app text-xs text-app font-mono">Authorization: Bearer YOUR_API_KEY</div>
                    </div>

                    <div>
                        <p className="text-[11px] text-muted mb-1">Content-Type</p>

                        <div className="px-3 py-2 rounded-lg bg-surface border border-app text-xs text-app font-mono">application/json</div>
                    </div>
                </div>
            </div>
        </div>
