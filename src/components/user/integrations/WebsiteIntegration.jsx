...
import { CheckCircle2, XCircle, Loader2, Copy, Check, Link2, RefreshCw, ArrowRight } from 'lucide-react';
...
export default function WebsiteIntegration({ integration: initialIntegration }) {
    ...
    const isConnected = integration?.provider === 'website' && integration?.status === 'connected';
    ...
    const brandBnaloSellerId = String(integration?.metadata?.brandBnaloSellerId || '');

    // Endpoint external systems can POST to (kept for display purposes)
    const endpoint = typeof window !== 'undefined' ? `${window.location.origin}/api/leads/ingest` : '/api/leads/ingest';
    ...
    const handleConnect = async () => {
        ...
        try {
            ...
            if (!response.data?.success) {
                throw new Error(response.data?.message || 'Failed to connect website');
            }
            ...
            toast.error(error?.response?.data?.message || error?.message || 'Failed to connect website');
        } finally {
            setConnecting(false);
        }
    };
    ...
    const handleDisconnect = async () => {
        try {
            ...
            if (!response.data?.success) {
                throw new Error(response.data?.message || 'Failed to disconnect website');
            }
            ...
            toast.error(error?.response?.data?.message || error?.message || 'Failed to disconnect website');
        } finally {
            setConnecting(false);
        }
    };
    ...
    const handleSync = async () => {
        try {
            ...
            const { imported = 0, skipped = 0, failed = 0 } = response.data || {};

            if (failed > 0) {
                toast.error(`Synced with issues — imported ${imported}, skipped ${skipped}, failed ${failed}`);
            } else if (imported > 0) {
                toast.success(`Imported ${imported} new lead${imported > 1 ? 's' : ''}`);
            } else {
            ...
            toast.error(error?.response?.data?.message || error?.message || 'Failed to sync website leads');
        } finally {
            setSyncing(false);
        }
    };
        ...
        <div className="bg-app border-app overflow-hidden rounded-2xl border">
            ...
            <div className="border-app flex items-center justify-between border-b px-5 py-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-app text-sm font-semibold">Website Lead Connection</h3>
                        <p className="text-muted mt-0.5 text-xs">Sync leads from your BrandBnalo website forms</p>
                    </div>
                </div>
                ...
                <div className="space-y-4 p-5">
                    <div className="border-app bg-surface rounded-xl border p-5">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <h4 className="text-app text-sm font-semibold">Connect your BrandBnalo website</h4>
                                <p className="text-muted mt-1 text-xs leading-5">
                                    Paste the Seller ID from your BrandBnalo dashboard. Once connected, your website form submissions will sync into this CRM
                        ...
                        {/* SELLER ID INPUT */}
                        <div className="mt-5">
                            <label className="text-app text-xs font-medium">BrandBnalo Seller ID</label>
                            <p className="text-muted mt-0.5 text-[11px]">Find this on your BrandBnalo dashboard under Settings → Seller ID.</p>
                            ...
                            <input
                                ...
                                disabled={connecting}
                                className="border-app bg-app text-app mt-2 w-full rounded-lg border px-3 py-2.5 font-mono text-xs transition outline-none focus:border-blue-500 disabled:opacity-60"
                            />
                        </div>
                ...
                <div className="space-y-4 p-5">
                    ...
                    <div>
                        ...
                        <div className="mt-2 flex gap-2">
                            <div className="border-app bg-surface text-app min-w-0 flex-1 rounded-lg border px-3.5 py-2.5 font-mono text-xs break-all">
                                {brandBnaloSellerId || '—'}
                            </div>
                            <button
                                onClick={() => copyText(brandBnaloSellerId, 'sellerId')}
                            ...
                            >
                                {copied === 'sellerId' ? <Check size={16} className="text-green-600" /> : <Copy size={16} />}
                            </button>
                        </div>
                    </div>
                    ...
                    {/* CRM USER ID */}
                    <div>
                        <p className="text-app text-sm font-medium">CRM User ID</p>
                        <p className="text-muted mt-0.5 text-xs">Your CRM company identifier.</p>
                        ...
                        <div className="mt-2 flex gap-2">
                            ...
                            <button
                                ...
                                className="border-app bg-app hover-app text-muted flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border disabled:opacity-50"
                            >
                                {copied === 'userId' ? <Check size={16} className="text-green-600" /> : <Copy size={16} />}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="border-app bg-surface flex items-center justify-between gap-3 rounded-xl border px-4 py-3">
                        <div className="min-w-0">
                            <p className="text-app text-xs font-medium">Auto-sync every minute</p>
                            <p className="text-muted mt-0.5 text-[11px]">
                                {integration?.lastSyncAt ? `Last sync: ${new Date(integration.lastSyncAt).toLocaleString('en-IN')}` : 'Not synced yet'}
                            </p>
                        </div>
                    ...
                    {integration?.errorMessage && (
                        <div className="rounded-xl border border-red-500/20 bg-red-500/5 px-4 py-3">
                            <p className="text-xs font-medium text-red-600">Sync error</p>
                            <p className="mt-0.5 text-[11px] break-words text-red-500/90">{integration.errorMessage}</p>
                        </div>
                    )}
                    ...
                    <details className="border-app rounded-xl border">
                        ...
                        <div className="border-app border-t px-4 py-3">
                            ...
                            <div className="text-app mb-2 rounded-lg bg-gray-900 px-3 py-2 font-mono text-[11px] break-all text-gray-200">POST {endpoint}</div>

                            <div className="mb-2 flex items-center justify-between">
                                <p className="text-muted text-[11px]">Payload:</p>
                                <button
                    ...
                    <div className="border-app flex items-center justify-between border-t pt-4">
                        <div>
                            <p className="text-app text-xs font-medium">Website connection</p>
                            <p className="text-muted mt-0.5 text-[11px]">Disconnect to stop receiving website leads.</p>
                        </div>
                        <button
