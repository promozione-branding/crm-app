...
export default function Team() {
    ...
    const getUsers = async () => {
        try {
            ...
            const res = await axios.get(`/api/user?page=${page}&limit=${rowsPerPage}&search=${search}`, {
    ...
    const handleCreateUser = async () => {
        // Required fields
        if (!form.name.trim() || !form.email.trim() || !form.phone.trim() || !form.password.trim() || !form.roleId) {
    ...
    return (
        <div className="bg-surface min-h-screen text-app">
            ...
            {active === 'role' && <Roles roles={roles} fetchRoles={fetchRoles} loading={loading1} />}
        </div>
    );
}
