...
export async function GET(request) {
    try {
        ...
        if (error) {
            ...
            return NextResponse.redirect(new URL('/integration?google=error', request.url));
        }
        ...
        try {
            decodedState = jwt.verify(state, ENV.JWT_CLIENT_SECRET);
        } catch (error) {
            console.error('Google OAuth state verification failed:', error.message);
        ...
        // when reconnecting.
        const finalRefreshToken = refresh_token || existingIntegration?.credentials?.refreshToken || null;
        ...
        await Integration.findOneAndUpdate(
            ...
            },
            {
                $set: {
                    ...
                    connectedAt: existingIntegration?.connectedAt || new Date(),
        ...
        // -----------------------------------------
        const response = NextResponse.redirect(new URL('/integration?google=connected', request.url));
