// src/app/api/user/google/accounts/details/route.js
...
export async function GET(req) {
    try {
        ...
        console.log('Google Ads resource names:', resourceNames);

        const customerIds = resourceNames.map((resourceName) => resourceName.replace('customers/', ''));

        console.log('Google Ads customer IDs:', customerIds);
        ...
        for (const customerId of customerIds) {
            try {
                ...
                console.log(`Customer ${customerId} result:`, result);
                ...
                if (result?.length) {
                    ...
                    accounts.push({
                        ...
                        currency: row.customer.currency_code || null,
                ...
                console.error(`Customer ${customerId} failed:`, accountError?.errors || accountError?.message || accountError);
            }
        }

        console.log('Final Google Ads accounts:', accounts);
        ...
        console.error('Google Ads Account Details Error:', error);

        return NextResponse.json(
            {
                success: false,
                message: error?.errors?.[0]?.message || error?.message || 'Failed to fetch Google Ads account details',
            },
            { status: 500 }
        );
    }
