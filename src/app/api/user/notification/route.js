...
export async function GET(request) {
    try {
        ...
        const [leads, tasks, unreadCount] = await Promise.all([
            Notification.find({ recipient: user._id, refModel: 'Lead' }).sort({ isRead: 1, createdAt: -1 }).limit(20).lean(),

            Notification.find({ recipient: user._id, refModel: 'LeadTask' }).sort({ isRead: 1, createdAt: -1 }).limit(20).lean(),
