    ...
    try {
        ...
        if (!refId || !phoneNumber) {
            return NextResponse.json(
                { success: false, message: 'refId and phoneNumber are required.' },
                { status: 400 }
            );
        }
        ...
        return NextResponse.json(
            { success: true, message: 'Call logged.', data: call },
            { status: 201 }
        );
    } catch (error) {
        console.error('CREATE CALL ERROR:', error);
        return NextResponse.json(
            { success: false, message: error.message || 'Failed to log call.' },
            { status: 400 }
        );
    }
}
...
export async function GET(request) {
    try {
        ...
        return NextResponse.json(
            { success: false, message: error.message || 'Failed to fetch calls.' },
            { status: 400 }
        );
    }
}