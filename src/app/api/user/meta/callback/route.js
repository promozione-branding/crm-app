...
export async function GET(request) {
    try {
        ...
        try {
            ...
            console.error('META OAUTH STATE ERROR:', error.message);
            return NextResponse.json({ success: false, message: 'Invalid OAuth state' }, { status: 400 });
        }

        const { userId, companyId } = oauthData;
        console.log('META OAUTH STATE VERIFIED:', { userId, companyId });
        const company = await Company.findById(companyId).select('crmDomain name').lean();
        ...
        if (!company.crmDomain) {
            return NextResponse.json({ success: false, message: 'Company CRM domain not configured' }, { status: 400 });
        }

        // Normalize CRM domain
        const crmDomain = company.crmDomain.replace(/^https?:\/\//, '').replace(/\/+$/, '');
        ...
        const tokenResponse = await fetch(`https://graph.facebook.com/${META_API_VERSION}/oauth/access_token?${tokenParams.toString()}`, { method: 'GET' });
        ...
        console.log('META CONNECTED SUCCESSFULLY', { userId, companyId, crmDomain });

        // Redirect to client's CRM
        const redirectUrl = `https://${crmDomain}/integration`;
